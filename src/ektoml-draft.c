...
#define GET_PTR(_ptr_with_flag) ((void *)((uintptr_t)(_ptr_with_flag) & ~0x1ull))
#define GET_FLAG(_ptr_with_flag) ((uintptr_t)(_ptr_with_flag) & 0x1ull)
...
static bool parse_keyvalue(parser_t *parser, toml_table_info_t *table);
static bool parse_value(parser_t *parser, void *restrict parent_data,
			const toml_val_t *val, void **data_out);

static void set_errf(parser_t *parser, const char *msg, ...) {
	char *buf = (char *)parser->arena_ptr;
	if (parser->arena_ptr + 64 > parser->arena_end) {
		SET_ERR(parser, msg);
		return;
	}
...
static toml_table_info_t *keyid_add_table_info(parser_t *parser,
	...
	table->_kv[keyid] = PTR_WITH_FLAG(new_table, false);
	return new_table;
}
...
static bool parse_table(parser_t *parser, void *restrict parent_data,
			const toml_val_t *val, void **data_out) {
	parser->src++, parser->pos++;
	uint8_t *const arena_ptr = parser->arena_ptr;

	if (val->type != TOML_TABLE) {
		set_err_type(parser, val->type);
		return false;
	}
	const toml_table_info_t info = val->load_table(parent_data);
	if (!info.start) return false;
	toml_table_info_t *table = add_table_info(parser, info);

	parse_whitespace(parser, true);
	while (*parser->src != '}') {
		parse_whitespace(parser, true);
		if (!parse_keyvalue(parser, table)) return false;
		parse_whitespace(parser, true);
	}

	// Make sure the table is fully defined
	for (int i = 0; i < table->len; i++) {
		if (GET_FLAG(table->_kv[i]) || table->start[i].val.optional) continue;
		set_errf(parser, "table entry %s not found",
			table->start[i].name);
		return false;
	}
	
	*data_out = PTR_WITH_FLAG(NULL, true);
	parser->arena_ptr = arena_ptr;
	return false;
}
...
static bool parse_array(parser_t *parser, void *restrict parent_data,
	...
	toml_array_info_t array = val->load_array(parent_data);
	...
	do {
		void *elem_val;
		if (!parse_value(parser, parent_data, array.type, &elem_val)) {
			return false;
		}
	...
	if (*parser->src != ']') {
		set_errf(parser, "array of length %zu expected!", array.len);
		return false;
	}
...
static inline bool can_load_val(toml_table_info_t *table,
				void **val,
				const toml_val_t *toml_val) {
	if (GET_FLAG(*val)) return false;
	if (table->_num_total_set == table->len) return false;
	if (table->_num_req_set == table->num_required) return false;
	return true;
}

static bool mark_set(parser_t *parser, toml_table_info_t *table, ) {
...
static bool parse_keyvalue(parser_t *parser, toml_table_info_t *table) {
	...
	void **const val = &table->_kv[keyid];
	const toml_val_t *const toml_val = &table->start[keyid].val;
	if (!can_load_val(table, val, toml_val)) {
		SET_ERR(parser, "key already created");
		return false;
	}

	if (!parse_value(parser, table->data, toml_val, val)) return false;
	return mark_set(table, keyid, val);
}
...
static bool parse_line(parser_t *parser) {
	...
	case '\0': return false;
	default:
		if (!parse_keyvalue(parser, parser->table)) return false;
		parse_whitespace(parser, false);
		if (*parser->src != '\n' || *parser->src != '\0') {
